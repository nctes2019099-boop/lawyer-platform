...
// Ensure the @prisma/client package has an ESM `wasm.mjs` entry. Some installs
// only ship the CJS `wasm.js` shim, which makes ESM/Turbopack resolution of
// `@prisma/client/wasm` fail with "Module not found".
const clientDir = path.join(here, "..", "node_modules", "@prisma", "client");
const esmShim = path.join(clientDir, "wasm.mjs");
const esmContent = `// ESM entry shim (auto-generated by patch-prisma-wasm.mjs) — re-exports the CJS WASM client.
...
`;
if (!existsSync(esmShim)) {
  writeFileSync(esmShim, esmContent, "utf8");
  console.log("[patch-prisma-wasm] created @prisma/client/wasm.mjs ESM shim.");
} else {
  console.log("[patch-prisma-wasm] @prisma/client/wasm.mjs already exists.");
}
